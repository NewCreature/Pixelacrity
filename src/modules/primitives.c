...
void pa_do_line(int x1, int y1, int x2, int y2, void * dp, void (*proc)(int, int, int, int, void *))
{
	...
	if(brush_data->bitmap)
	{
		cx = al_get_bitmap_width(brush_data->bitmap) / 2;
		cy = al_get_bitmap_height(brush_data->bitmap) / 2;
	}
...
void pa_do_ellipse(int ix, int iy, float rx0, float ry0, void * dp, void (*proc)(int, int, int, int, void *))
{
	BRUSH_DATA * brush_data = (BRUSH_DATA *)dp;
	int rx = 0, ry = 0;
	int x, y;
   ...
   /* First set of points. */
   while (y <= ry) {
      proc(ix + x, iy + y, cx, cy, dp);
      if (x != 0) {
         proc(ix - x + shift_x, iy + y, cx, cy, dp);
      }
      if (y != 0) {
         proc(ix + x, iy - y + shift_y, cx, cy, dp);
         if (x != 0) {
            proc(ix - x + shift_x, iy - y + shift_y, cx, cy, dp);
         }
      }
   ...
   while (x < midway_x) {
	 ...
	 if(shift_x)
	 {
		 proc(ix + shift_x, iy - ry + shift_y, cx, cy, dp);
		 proc(ix + shift_x, iy + ry, cx, cy, dp);
	 }
	 if(shift_y)
	 {
		 proc(ix - rx + shift_x, iy + shift_y, cx, cy, dp);
		 proc(ix + rx, iy + shift_y, cx, cy, dp);
	 }
}

static void brush_draw(int x, int y, int cx, int cy, void * dp)
{
	BRUSH_DATA * brush_data = (BRUSH_DATA *)dp;
