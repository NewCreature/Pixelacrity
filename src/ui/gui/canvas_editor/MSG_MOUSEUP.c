	...
	switch(canvas_editor->current_tool)
	{
		...
		case PA_TOOL_RECTANGLE:
		{
			...
			if(canvas_editor->tool_state == PA_TOOL_STATE_DRAWING && c == canvas_editor->click_button)
			{
				made_undo = create_primitive_undo(canvas_editor);
				handle_canvas_expansion(canvas_editor);
				pa_draw_primitive_to_canvas(canvas_editor->canvas, canvas_editor->current_layer, start_x, start_y, end_x, end_y, canvas_editor->brush, canvas_editor->click_color, PA_RENDER_COPY, canvas_editor->conditional_copy_shader, pa_draw_rectangle);
				al_use_shader(canvas_editor->standard_shader);
				if(made_undo)
				{
					pa_finalize_undo(canvas_editor);
		...
		case PA_TOOL_FILLED_RECTANGLE:
		{
			...
			if(canvas_editor->tool_state == PA_TOOL_STATE_DRAWING && c == canvas_editor->click_button)
			{
				...
				pa_draw_primitive_to_canvas(canvas_editor->canvas, canvas_editor->current_layer, start_x, start_y, end_x, end_y, canvas_editor->brush, canvas_editor->click_outline_color, PA_RENDER_COPY, canvas_editor->conditional_copy_shader, pa_draw_rectangle);
				al_use_shader(canvas_editor->standard_shader);
				if(made_undo)
				{
					pa_finalize_undo(canvas_editor);
		...
		case PA_TOOL_FILLED_OVAL:
		{
			...
			if(canvas_editor->tool_state == PA_TOOL_STATE_DRAWING && c == canvas_editor->click_button)
			{
				...
				pa_draw_primitive_to_canvas(canvas_editor->canvas, canvas_editor->current_layer, start_x, start_y, end_x, end_y, canvas_editor->brush, canvas_editor->click_outline_color, PA_RENDER_COPY, canvas_editor->conditional_copy_shader, pa_draw_oval);
				al_use_shader(canvas_editor->standard_shader);
				if(made_undo)
				{
					pa_finalize_undo(canvas_editor);
