...
static bool add_color_pickers(PA_DIALOG * dp, PA_CANVAS_EDITOR * cep, int x, int y)
{
	...
	{
		t3gui_dialog_add_element(dp->dialog, dp->theme->theme[PA_UI_THEME_COLOR_SELECT], pa_gui_color_proc, 0, 0, 0, 0, 0, 0, 0, 0, pa_create_gui_color_data(&cep->left_shade_color[i], NULL, &cep->left_color, &cep->right_color, NULL, NULL, NULL, NULL), NULL, NULL);

	}
	dp->variable[PA_UI_VAR_LEFT_ALPHA_PICKER_INDEX] = dp->dialog->elements;
	for(i = 0; i < PA_COLOR_PICKER_SHADES; i++)
	{
		t3gui_dialog_add_element(dp->dialog, dp->theme->theme[PA_UI_THEME_COLOR_SELECT], pa_gui_color_proc, 0, 0, 0, 0, 0, 0, 0, 0, pa_create_gui_color_data(&cep->left_alpha_color[i], NULL, &cep->left_color, &cep->right_color, NULL, NULL, NULL, NULL), NULL, NULL);
	}
	dp->variable[PA_UI_VAR_RIGHT_SHADE_PICKER_INDEX] = dp->dialog->elements;
	for(i = 0; i < PA_COLOR_PICKER_SHADES; i++)
	{
		t3gui_dialog_add_element(dp->dialog, dp->theme->theme[PA_UI_THEME_COLOR_SELECT], pa_gui_color_proc, 0, 0, 0, 0, 0, 0, 0, 0, pa_create_gui_color_data(&cep->right_shade_color[i], NULL, &cep->left_color, &cep->right_color, NULL, NULL, NULL, NULL), NULL, NULL);
	}
	dp->variable[PA_UI_VAR_RIGHT_ALPHA_PICKER_INDEX] = dp->dialog->elements;
	for(i = 0; i < PA_COLOR_PICKER_SHADES; i++)
	{
		t3gui_dialog_add_element(dp->dialog, dp->theme->theme[PA_UI_THEME_COLOR_SELECT], pa_gui_color_proc, 0, 0, 0, 0, 0, 0, 0, 0, pa_create_gui_color_data(&cep->right_alpha_color[i], NULL, &cep->left_color, &cep->right_color, NULL, NULL, NULL, NULL), NULL, NULL);
	}
...
static void add_left_pane(PA_DIALOG * dp, PA_CANVAS_EDITOR * cep)
{
	...
	ep = t3gui_dialog_add_element(dp->dialog, dp->theme->theme[PA_UI_THEME_COLOR_SELECT], pa_gui_color_proc, 0, 0, 0, 0, 0, 0, 1, 0, pa_create_gui_color_data(NULL, &cep->left_color, NULL, NULL, NULL, NULL, NULL, NULL), NULL, NULL);
	pa_track_dialog_element(dp, ep, PA_UI_ELEMENT_LEFT_COLOR);

	ep = t3gui_dialog_add_element(dp->dialog, dp->theme->theme[PA_UI_THEME_COLOR_SELECT], pa_gui_color_proc, 0, 0, 0, 0, 0, 0, 1, 0, pa_create_gui_color_data(NULL, &cep->right_color, NULL, NULL, NULL, NULL, NULL, NULL), NULL, NULL);
